...
| M2  | **创建 skill 入口** — `/decay` slash command + SKILL.md | 用户在 Claude Code 中 `/decay` 触发健康检查并看到格式化结果     | 1/1 | [skill-entry](../requirements/skill-entry/prd.md) |
| M3  | **添加 Markdown 输出** — `--markdown` flag 生成人可读报告      | `decay --markdown` 输出包含评分、趋势、问题、处方的格式化报告      | 1/1 | [markdown-output](../requirements/markdown-output/prd.md) |
| M4  | **通用集成** — `--quiet` + 语义化 exit code              | `decay --quiet` 一行摘要，exit 0=ok/1=critical，任何工具可集成 | 1/1 | [quiet-mode](../requirements/quiet-mode/prd.md)   |
