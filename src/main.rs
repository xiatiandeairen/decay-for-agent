...
const HEALTH_REPORT_TEMPLATE: &str = include_str!("../templates/health-report.md");
...
struct MarkdownCtx<'a> {
    ...
    f: Option<i32>,
    comp: i32,
    trend_data: &'a Option<trend::Trend>,
    scan_summary: &'a scan::ScanSummary,
    git_summary: &'a Option<git::GitSummary>,
    issues: &'a [diagnose::Issue],
}

fn render_markdown(ctx: &MarkdownCtx<'_>) -> String {
    let MarkdownCtx {
        snapshot_id,
        project_path,
        s,
...
fn main() -> Result<()> {
    ...
    let git_summary = match git::collect(&conn, snapshot_id, &project_path, 90) {
        ...
        Err(e) => {
            ...
            if !cli.json && !cli.markdown && !cli.quiet {
